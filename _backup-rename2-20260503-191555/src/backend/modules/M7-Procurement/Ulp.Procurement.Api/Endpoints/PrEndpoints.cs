...
public static class PrEndpoints
{
    ...
    {
        var g = app.MapGroup("/api/v1/procurement/purchase-requests").WithTags("M7 Â· Purchase Requests").RequireAuthorization();
        ...
        g.MapPost("/", async (
            ...
            [FromServices] IProcurementService svc, CancellationToken ct) =>
        {
            try   { return Results.Created("/api/v1/procurement/purchase-requests/", await svc.CreatePrAsync(req, ct)); }
        ...
        g.MapPost("/{id:long}/lines", async (long id,
            ...
            [FromServices] IProcurementService svc, CancellationToken ct) =>
        {
            try   { return Results.Created($"/api/v1/procurement/purchase-requests/{id}/lines", await svc.AddPrLineAsync(id, req, ct)); }
