...
public static class PoEndpoints
{
    ...
    {
        var g = app.MapGroup("/api/v1/procurement/purchase-orders").WithTags("M7 Â· Purchase Orders").RequireAuthorization();
        ...
        g.MapPost("/", async (
            ...
            [FromServices] IProcurementService svc, CancellationToken ct) =>
        {
            try   { return Results.Created("/api/v1/procurement/purchase-orders/", await svc.CreatePoAsync(req, ct)); }
        ...
        g.MapPost("/{id:long}/lines", async (long id,
            ...
            [FromServices] IProcurementService svc, CancellationToken ct) =>
        {
            try   { return Results.Created($"/api/v1/procurement/purchase-orders/{id}/lines", await svc.AddPoLineAsync(id, req, ct)); }
