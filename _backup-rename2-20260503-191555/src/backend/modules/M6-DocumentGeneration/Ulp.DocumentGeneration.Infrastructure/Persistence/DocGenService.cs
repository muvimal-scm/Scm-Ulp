...
using Ulp.DocumentGeneration.Domain.Entities;
using RenderRequestEntity = Ulp.DocumentGeneration.Domain.Entities.RenderRequest;
using RendererRequest    = Ulp.Core.Abstractions.Rendering.RenderRequest;

namespace Ulp.DocumentGeneration.Infrastructure.Persistence;

/// <summary>
/// Phase 2.0 doc generation:
///   1. Resolve template by code (+ optional country variant) â€” fall back to system template (tenant_id=NULL)
...
public sealed class DocGenService(
    DocumentGenerationDbContext db,
    ...
    IClock clock) : IDocGenService
{
    ...
    public async Task<TemplateVersionDto> AddTemplateVersionAsync(long templateId, AddVersionRequest req, CancellationToken ct)
    {
        ...
        if (template.TenantId is null)
            throw new InvalidOperationException("system templates cannot be edited directly â€” fork to a tenant template first");
