...
public static class ReadOnlyEndpoints
{
    ...
    {
        var stages = app.MapGroup("/api/v1/sales/pipeline-stages").WithTags("M2 Â· Pipeline").RequireAuthorization();
        stages.MapGet("/", async ([FromServices] ICrmService svc, CancellationToken ct) =>
            Results.Ok(await svc.ListPipelineStagesAsync(ct)));

        var fc = app.MapGroup("/api/v1/sales/forecasts").WithTags("M2 Â· Forecasts").RequireAuthorization();
        fc.MapGet("/", async (
