...
public static class LastMileEndpoints
{
    ...
    {
        var bookings = app.MapGroup("/api/v1/last-mile/bookings").WithTags("M9 Â· Courier Bookings").RequireAuthorization();
        ...
        var routes = app.MapGroup("/api/v1/last-mile/routes").WithTags("M9 Â· Routes").RequireAuthorization();
        ...
        var manifests = app.MapGroup("/api/v1/last-mile/manifests").WithTags("M9 Â· Manifests").RequireAuthorization();
        manifests.MapGet("/", async ([FromServices] ILastMileService svc, CancellationToken ct) =>
            Results.Ok(await svc.ListManifestsAsync(ct)));

        var pods = app.MapGroup("/api/v1/last-mile/pods").WithTags("M9 Â· POD").RequireAuthorization();
        ...
        var cods = app.MapGroup("/api/v1/last-mile/cod").WithTags("M9 Â· COD").RequireAuthorization();
        ...
        var attempts = app.MapGroup("/api/v1/last-mile/attempts").WithTags("M9 Â· Attempts").RequireAuthorization();
        ...
        var zones = app.MapGroup("/api/v1/last-mile/zone-rates").WithTags("M9 Â· Zone Rates").RequireAuthorization();
        zones.MapGet("/", async (
            [FromServices] ILastMileService svc,
            [FromQuery] string? countryCode,
            [FromQuery] CourierType? courierType,
