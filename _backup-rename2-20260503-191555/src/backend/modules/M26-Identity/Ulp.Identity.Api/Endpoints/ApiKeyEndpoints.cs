...
public static class ApiKeyEndpoints
{
    ...
    {
        var g = app.MapGroup("/api/v1/identity/api-keys").WithTags("M26 Â· API Keys").RequireAuthorization();

        g.MapGet("/", async ([FromServices] IdentityDbContext db, [FromServices] ITenantContext tenant,
            ...
            CancellationToken ct) =>
        {
            ...
            return Results.Created($"/api/v1/identity/api-keys/{key.Id}", new
            {
                id = key.Id,
                name = key.Name,
                rawKey = raw,
