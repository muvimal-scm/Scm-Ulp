...
namespace Ulp.Identity.Infrastructure.Tenancy;

/// <summary>
/// Per LLD Â§5: extract tenant_id from JWT, look up the tenant record, verify the
...
/// using IRegionResolver â€” Phase 1 dev defers this to a single-region setup).
/// </summary>
    ...
    ILogger<KeycloakTenantContextMiddleware> log)
{
        ...
        IClock clock)
    {
        ...
        // Map sub â†’ local user id (and record the login on first hit).
        long? userId = null;
