...
public static class ReadOnlyEndpoints
{
    ...
    {
        var consols = app.MapGroup("/api/v1/freight-forwarding/consols").WithTags("M5 Â· Consols").RequireAuthorization();
        consols.MapGet("/", async ([FromServices] IFreightService svc, CancellationToken ct) =>
            Results.Ok(await svc.ListConsolsAsync(ct)));

        var demu = app.MapGroup("/api/v1/freight-forwarding/demurrage").WithTags("M5 Â· Demurrage").RequireAuthorization();
        demu.MapGet("/", async (
