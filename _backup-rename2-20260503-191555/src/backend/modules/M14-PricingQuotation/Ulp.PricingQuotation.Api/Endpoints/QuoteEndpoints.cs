...
public static class QuoteEndpoints
{
    ...
    {
        var g = app.MapGroup("/api/v1/pricing-quotation/quotes").WithTags("M14 Â· Quotes").RequireAuthorization();
        ...
        g.MapPost("/", async (
            ...
            [FromServices] IPricingService svc, CancellationToken ct) =>
        {
            try   { return Results.Created($"/api/v1/pricing-quotation/quotes/", await svc.CreateQuoteAsync(req, ct)); }
        ...
        g.MapPost("/{id:long}/lines", async (long id,
            ...
            [FromServices] IPricingService svc, CancellationToken ct) =>
        {
            try   { return Results.Created($"/api/v1/pricing-quotation/quotes/{id}/lines", await svc.AddQuoteLineAsync(id, req, ct)); }
            catch (InvalidOperationException ex) { return Results.Conflict(new { error = ex.Message }); }
        });
