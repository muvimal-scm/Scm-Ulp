...
import { SettlementLinkDto } from '../shared/accounting-ext-types';

@Component({
  selector: 'ulp-accounting-settlement',
  standalone: true,
  imports: [DecimalPipe, SlicePipe, MatIconModule, MatProgressSpinnerModule],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <header class="page-head">
      <h1>A/R â†” A/P Settlement</h1>
      <p>Cross-link an A/R invoice line to an A/P bill line â€” receivable stays trackable
          ...
          <tbody>
            @for (s of rows(); track s.id) {
              <tr>
                ...
                <td>{{ s.reversedAt ? (s.reversedAt | slice:0:16) : 'â€”' }}</td>
                <td class="muted small">{{ s.reversalReason ?? 'â€”' }}</td>
                <td>
                  @if (s.status === 'Active') {
