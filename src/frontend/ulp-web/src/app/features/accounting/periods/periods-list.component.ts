...
import { PeriodDto } from '../shared/accounting-types';

@Component({
  selector: 'ulp-accounting-periods',
  standalone: true,
  ...
  template: `
    <header class="page-head">
      ...
      <p>Fiscal periods (Apr-Mar for IN, Jan-Dec for US). Closed periods are immutable; reopening
         needs CFO+Auditor RBAC per LLD Â§7.4.</p>
    </header>
          ...
          <tbody>
            @for (p of periods(); track p.id) {
              <tr [class.row-open]="p.status === 'Open'" [class.row-closed]="p.status === 'Closed'">
                ...
                <td>{{ p.closedAt ? (p.closedAt | slice:0:16) : 'â€”' }}</td>
                <td>{{ p.reopenedCount > 0 ? p.reopenedCount : 'â€”' }}</td>
                <td class="actions">
                  @if (p.status === 'Open') {
                    <button class="btn-link" (click)="closePeriod(p)" [disabled]="busy()">Close period</button>
                  }
                  @if (p.status === 'Closed') {
                    <button class="btn-link" (click)="reopenPeriod(p)" [disabled]="busy()">Reopenâ€¦</button>
                  }
                </td>
              </tr>
            }
