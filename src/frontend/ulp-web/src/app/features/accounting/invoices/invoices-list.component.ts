...
import { InvoiceDto, InvoiceStatus } from '../shared/accounting-types';

// v2 client doc Tab 5: "Invoices: Shipments In-Transit". Approximated client-side
// as Posted + PartiallyPaid + Overdue (the practical "outstanding for shipped
// goods" states). Full backend-joined version is a CP13 deferred item once
// invoice → shipment FK is exposed in the DTO.
type FilterTab = 'All' | 'In-Transit' | InvoiceStatus;

@Component({
  selector: 'ulp-accounting-invoices',
  standalone: true,
  ...
  template: `
    ...
    <nav class="tabs">
      <button class="tab" [class.tab--active]="filter() === 'All'" (click)="filter.set('All')">All</button>
      <button class="tab tab--special" [class.tab--active]="filter() === 'In-Transit'" (click)="filter.set('In-Transit')">
        <mat-icon class="tab__ic">local_shipping</mat-icon>
  ...
  styles: [`
    ...
    .tab--special { display: inline-flex; align-items: center; gap: 6px; }
    .tab__ic { font-size: 16px; width: 16px; height: 16px; line-height: 16px; }
...
export class InvoicesListComponent implements OnInit {
  ...
  // In-Transit tab approximation: outstanding invoices on goods that have left the dock.
  private static readonly inTransitStatuses: InvoiceStatus[] = ['Posted', 'PartiallyPaid', 'Overdue'];

  readonly filtered = computed(() => {
    const f = this.filter();
    const all = this.invoices();
    if (f === 'All') return all;
    if (f === 'In-Transit') return all.filter(i => InvoicesListComponent.inTransitStatuses.includes(i.status));
