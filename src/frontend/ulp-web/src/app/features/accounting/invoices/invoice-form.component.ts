...
import {
  CreateInvoiceLineRequest, CreateInvoiceRequest, ExportType,
...
@Component({
  selector: 'ulp-acct-invoice-form',
  standalone: true,
  imports: [
  ...
  template: `
    <header class="page-head">
      ...
      <h1>New AR Invoice</h1>
      <p>Customer invoice — saved as Draft. Post to GL from the detail screen.</p>
    </header>
    ...
    <form class="form-card" [formGroup]="header">
      ...
      <div class="row-2">
        ...
        <mat-form-field appearance="outline">
          ...
          @if (header.get('invoiceNumber')?.hasError('required')) { <mat-error>Required</mat-error> }
          <mat-hint>Immutable; auto-suggested INV-YYYYMMDD</mat-hint>
        </mat-form-field>
      </div>
      ...
      <div class="row-2">
        <mat-form-field appearance="outline">
          ...
          <input matInput formControlName="paymentTerms" maxlength="50" placeholder="Net 30" />
        </mat-form-field>
        <mat-form-field appearance="outline">
          <mat-label>Place of supply (IN)</mat-label>
          <input matInput formControlName="placeOfSupply" maxlength="2" placeholder="MH, KA, DL" />
        </mat-form-field>
      </div>
      ...
      @if (lines().length > 0) {
        <table class="lines">
          ...
          <tbody>
            ...
            <tr class="total-row">
              <td colspan="7" class="num"><strong>Subtotal</strong></td>
    ...
    <div class="actions">
      <button mat-button type="button" routerLink="/app/accounting/invoices" [disabled]="saving()">Cancel</button>
      <button mat-flat-button color="primary" type="button" (click)="save()"
              [disabled]="saving() || header.invalid || lines().length === 0">
        @if (saving()) { Saving… } @else { Create Invoice }
      </button>
    </div>
  `,
  styles: [`
...
export class InvoiceFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.header.patchValue({
      invoiceNumber: `INV-${yyyymmdd}-${hhmm}`,
  ...
  addLine() {
    ...
    this.lines.update(arr => [...arr, { ...v, unitPriceCurrency: cur, accountId: null }]);
    this.newLine.reset({ description: '', hsnCode: '', quantity: 1, uomCode: 'EACH', unitPriceAmount: 0, unitPriceCurrency: cur });
  }
  ...
  async save() {
    ...
    const lineReqs: CreateInvoiceLineRequest[] = this.lines().map(l => ({
    ...
    const req: CreateInvoiceRequest = {
      ...
      isExport:        null,
      exportType:      null as ExportType | null,
    };

    try {
      const inv = await this.api.createInvoice(req);
      this.snack.open(`Invoice ${inv.invoiceNumber} created`, 'Dismiss', { duration: 4000 });
      await this.router.navigate(['/app/accounting/invoices', inv.id]);
    } catch (e: any) {
