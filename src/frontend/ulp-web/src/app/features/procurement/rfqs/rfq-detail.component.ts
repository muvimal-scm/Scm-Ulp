...
import { RfqDetailDto } from '../shared/procurement-types';

@Component({
  selector: 'ulp-procurement-rfq-detail',
  standalone: true,
  ...
  template: `
    ...
    @if (data(); as d) {
      <a routerLink=".." class="back">← Back to vendor RFQs</a>
      <header class="page-head">
        <h1>{{ d.rfq.rfqNumber }}</h1>
        <p>
          @if (d.rfq.scopePrId) { sourced from PR #{{ d.rfq.scopePrId }} · }
          due {{ d.rfq.dueDate ? (d.rfq.dueDate | slice:0:10) : '—' }}
          · <span class="status status--{{ d.rfq.status.toLowerCase() }}">{{ d.rfq.status }}</span>
        </p>
      </header>

      @if (d.rfq.notes) { <section class="card"><h2>Notes</h2><p>{{ d.rfq.notes }}</p></section> }

      <section class="card">
        <h2>Recipients ({{ d.recipients.length }})</h2>
        @if (d.recipients.length === 0) { <p class="muted">No recipients.</p> } @else {
          <table>
            <thead><tr><th>Vendor</th><th>Sent</th><th>Status</th></tr></thead>
            <tbody>
              @for (r of d.recipients; track r.id) {
                <tr>
                  <td>party #{{ r.vendorPartyId }}</td>
                  <td>{{ r.sentAt ? (r.sentAt | slice:0:16) : '—' }}</td>
                  <td><span class="recip recip--{{ r.responseStatus.toLowerCase() }}">{{ r.responseStatus }}</span></td>
                </tr>
              }
            </tbody>
          </table>
      ...
      <section class="card">
        ...
        @if (d.responses.length === 0) { <p class="muted">No responses received yet.</p> } @else {
          <table>
            <thead><tr>
              <th>Vendor</th><th class="num">Bid total</th>
              <th>Valid until</th><th>Received</th><th>Notes</th><th>Winner</th>
            </tr></thead>
            <tbody>
              @for (r of d.responses; track r.id) {
                <tr [class.winner]="r.isWinner">
                  <td>party #{{ r.vendorPartyId }}</td>
                  <td class="num">{{ r.totalAmount ? (r.totalAmount | number:'1.0-2') : '—' }} {{ r.totalCurrency ?? '' }}</td>
  ...
  styles: [`
    ...
    .recip { padding: 2px 8px; border-radius: 999px; font-size: 10px; font-weight: 700; }
    .recip--notsent      { background: #E0E0E0; color: #555; }
    .recip--sent         { background: #DCEAF8; color: #1F4E8A; }
    .recip--acknowledged { background: #FFF3D6; color: #946100; }
    .recip--responded    { background: #DCF5E4; color: #1F7A3D; }
    .recip--declined     { background: #FBE4E5; color: #B23F45; }
    .recip--expired      { background: #F5F5F5; color: #777; }
  `],
})
export class RfqDetailComponent implements OnInit {
  private readonly api = inject(ProcurementApiService);
  private readonly route = inject(ActivatedRoute);
  readonly data = signal<RfqDetailDto | null>(null);
  readonly loading = signal(true);
  readonly error = signal<string | null>(null);

  async ngOnInit() {
    const id = Number(this.route.snapshot.paramMap.get('id'));
    if (!Number.isFinite(id) || id <= 0) { this.error.set('invalid RFQ id'); this.loading.set(false); return; }
