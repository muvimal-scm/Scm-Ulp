...
import { PoDto } from '../shared/procurement-types';
    ...
    <header class="page-head">
      <div class="head-row">
        <div>
          ...
          <p>Issued POs — Draft → Approved → Sent → PartialReceipt → Closed.</p>
        </div>
    ...
    @if (loading()) {
      ...
      <div class="table-wrap">
        <table>
          <thead>
            <tr>
              <th>PO #</th><th>Country</th><th>Vendor</th><th>RFQ</th>
              <th class="num">Total</th><th>Expected</th>
              <th>Status</th><th class="num">Lines</th><th class="num">GRNs</th><th></th>
            </tr>
          </thead>
          <tbody>
            @for (p of rows(); track p.id) {
              <tr>
                ...
                <td>{{ p.countryCode }}</td>
                <td>party #{{ p.vendorPartyId }}</td>
                <td>{{ p.rfqId ? ('RFQ #' + p.rfqId) : '—' }}</td>
                <td class="num">{{ p.totalAmount ? (p.totalAmount | number:'1.0-2') : '—' }} {{ p.totalCurrency ?? '' }}</td>
              ...
              <tr><td colspan="10" class="empty">No purchase orders yet.</td></tr>
            }
          </tbody>
        </table>
      </div>
...
export class PosListComponent implements OnInit {
  ...
  readonly rows = signal<PoDto[]>([]);
  readonly loading = signal(true);
  readonly error = signal<string | null>(null);

  async ngOnInit() {
    try { this.rows.set(await this.api.listPos()); }
    catch (e: any) { this.error.set(e?.message ?? 'Failed to load POs'); }
