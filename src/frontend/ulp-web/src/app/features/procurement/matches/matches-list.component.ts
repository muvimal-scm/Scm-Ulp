...
import { InvoiceMatchDto } from '../shared/procurement-types';

@Component({
  selector: 'ulp-procurement-matches',
  standalone: true,
        ...
        <table>
          ...
          <tbody>
            @for (m of rows(); track m.id) {
              <tr>
                <td>PO #{{ m.poId }}</td>
                <td>{{ m.vendorInvoiceNo ?? 'â€”' }}</td>
                <td><span class="match match--{{ m.matchStatus.toLowerCase() }}">{{ m.matchStatus }}</span></td>
                <td class="num">{{ m.varianceAmount !== null ? (m.varianceAmount | number:'1.2-2') : 'â€”' }} {{ m.varianceCurrency ?? '' }}</td>
