...
import { HoldExamDto, PgaHoldDto } from '../shared/customs-types';

@Component({
  selector: 'ulp-customs-holds',
  standalone: true,
          ...
          <table>
            ...
            <tbody>
              @for (h of pgaHolds(); track h.id) {
                <tr>
                  <td><a [routerLink]="['/app/customs/entries', h.entryId]" class="link">{{ h.entryNumber ?? '#' + h.entryId }}</a></td>
                  <td><span class="pga pga--{{ h.pgaCode.toLowerCase() }}">{{ h.pgaCode }}</span></td>
                  <td class="mono">{{ h.holdReasonCode ?? 'â€”' }}</td>
                  ...
                  <td>{{ h.releasedAt ? (h.releasedAt | slice:0:16) : 'â€”' }}</td>
                </tr>
            ...
            <tbody>
              @for (h of cbpHolds(); track h.id) {
                <tr>
                  ...
                  <td>{{ h.examType !== 'Nil' ? h.examType : 'â€”' }}</td>
                  <td class="mono">{{ h.holdReasonCode ?? 'â€”' }}</td>
                  <td class="reason">{{ h.holdReasonText }}</td>
                  <td>{{ h.examSite ?? 'â€”' }}</td>
                  <td>{{ h.examAppointmentAt ? (h.examAppointmentAt | slice:0:16) : 'â€”' }}</td>
                  <td><span class="status status--{{ h.status.toLowerCase() }}">{{ h.status }}</span></td>
                </tr>
