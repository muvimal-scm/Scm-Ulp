...
import { IsfDto } from '../shared/customs-types';

@Component({
  selector: 'ulp-customs-isf',
  standalone: true,
  ...
  template: `
    <header class="page-head">
      ...
      <p>Per LLD Â§9 â€” 10 importer-supplied data elements + 2 carrier elements, must be
          ...
          <tbody>
            @for (i of rows(); track i.id) {
              <tr>
                ...
                <td>{{ i.sellerName ?? 'â€”' }}</td>
                <td>{{ i.buyerName ?? 'â€”' }}</td>
                <td class="mono">{{ i.countryOfOrigin ?? 'â€”' }}</td>
                <td class="mono">{{ i.hts6 ?? 'â€”' }}</td>
                <td>{{ i.filedAt ? (i.filedAt | slice:0:16) : 'â€”' }}</td>
                <td>{{ i.vesselLoadCutoff ? (i.vesselLoadCutoff | slice:0:16) : 'â€”' }}</td>
