...
import { InBondDto } from '../shared/customs-types';

@Component({
  selector: 'ulp-customs-in-bond',
  standalone: true,
          ...
          <tbody>
            @for (i of rows(); track i.id) {
              <tr>
                ...
                <td>{{ i.arrivedAt ? (i.arrivedAt | slice:0:10) : 'â€”' }}</td>
                <td><span class="status status--{{ i.status.toLowerCase() }}">{{ i.status }}</span></td>
                <td>@if (i.entryId) { <a [routerLink]="['/app/customs/entries', i.entryId]" class="link">#{{ i.entryId }}</a> } @else { <span class="muted">â€”</span> }</td>
              </tr>
