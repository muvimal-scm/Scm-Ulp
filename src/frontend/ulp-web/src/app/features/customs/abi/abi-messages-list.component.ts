...
import { AbiMessageDto } from '../shared/customs-types';

@Component({
  selector: 'ulp-customs-abi-messages',
  standalone: true,
          ...
          <tbody>
            @for (m of rows(); track m.id) {
              <tr>
                ...
                <td>@if (m.entryId) { <a [routerLink]="['/app/customs/entries', m.entryId]" class="link">#{{ m.entryId }}</a> } @else { — }</td>
