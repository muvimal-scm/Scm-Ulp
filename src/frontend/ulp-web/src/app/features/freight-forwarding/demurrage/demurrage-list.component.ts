...
import { DemurrageEventDto } from '../shared/freight-forwarding-types';

@Component({
  selector: 'ulp-freight-forwarding-demurrage',
  standalone: true,
          ...
          <tbody>
            @for (d of rows(); track d.id) {
              <tr>
                ...
                <td class="num">{{ d.days ?? 'â€”' }}</td>
                <td class="num">{{ d.rateAmount ? (d.rateAmount | number:'1.2-2') : 'â€”' }} {{ d.rateCurrency ?? '' }}</td>
                <td class="num">{{ d.totalAmount ? (d.totalAmount | number:'1.2-2') : 'â€”' }} {{ d.totalCurrency ?? '' }}</td>
