...
import {
  ...
  ContainerDto, CreateBookingRequest, CreateContainerRequest,
  CreateHblRequest, CreateShipmentRequest,
  DemurrageEventDto, HblDto, HoldType, LandedCostDto, ReminderKind, ReminderStatus,
...
} from './freight-forwarding-types';

@Injectable({ providedIn: 'root' })
export class FreightForwardingApiService {
  private readonly http = inject(HttpClient);
  private readonly base = `${environment.apiBaseUrl}/api/v1/freight-forwarding`;
  ...
  createBooking(req: CreateBookingRequest): Promise<BookingDto> {
    return firstValueFrom(this.http.post<BookingDto>(`${this.base}/bookings`, req));
  }
  ...
  /* Shipments â€” Control Tower passes the rich filter set; basic callers pass status+mode */
  listShipments(opts: {
  ...
  /* SCM Milestone 1+2 â€” watchlist (current user) */
  ...
  createShipment(req: CreateShipmentRequest): Promise<ShipmentDto> {
    return firstValueFrom(this.http.post<ShipmentDto>(`${this.base}/shipments`, req));
  }
  ...
  addShipmentContainer(shipmentId: number, req: CreateContainerRequest): Promise<ContainerDto> {
    return firstValueFrom(this.http.post<ContainerDto>(`${this.base}/shipments/${shipmentId}/containers`, req));
  }

  createHbl(req: CreateHblRequest): Promise<HblDto> {
    return firstValueFrom(this.http.post<HblDto>(`${this.base}/hbls`, req));
  }
  ...
  getLandedCost(id: number): Promise<LandedCostDto> {
    return firstValueFrom(this.http.get<LandedCostDto>(`${this.base}/shipments/${id}/landed-cost`));
  }
