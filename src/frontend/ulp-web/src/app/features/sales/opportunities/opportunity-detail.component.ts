...
import { OpportunityDetailDto } from '../shared/sales-types';

@Component({
  selector: 'ulp-sales-opportunity-detail',
  standalone: true,
        ...
        @if (d.linkedQuoteIds.length === 0) {
          ...
          <ul class="link-list">
            @for (qid of d.linkedQuoteIds; track qid) {
              <li><a [routerLink]="['/app/pricing-quotation/quotes', qid]">Quote #{{ qid }} →</a></li>
            }
          </ul>
        }
      </section>
