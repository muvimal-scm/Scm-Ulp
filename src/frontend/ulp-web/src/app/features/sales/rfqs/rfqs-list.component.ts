...
import { RfqRequestDto } from '../shared/sales-types';

@Component({
  selector: 'ulp-sales-rfqs',
  standalone: true,
  ...
  template: `
    <header class="page-head">
      <div class="head-row">
        <div>
          <h1>RFQs</h1>
          <p>Customer-driven request-for-quote workflow with vendor responses.</p>
        </div>
        <a mat-flat-button color="primary" routerLink="new"><mat-icon>add</mat-icon>&nbsp;New RFQ</a>
      </div>
    </header>
    ...
    @if (loading()) {
      ...
      <div class="table-wrap">
        <table>
          <thead>
            <tr>
              <th>RFQ #</th><th>Party</th><th>Requested</th><th>Due</th>
              <th>Status</th><th class="num">Lines</th><th class="num">Responses</th><th></th>
            </tr>
          </thead>
          <tbody>
            @for (r of rows(); track r.id) {
              <tr>
                ...
                <td><a [routerLink]="[r.id]" class="link">Open →</a></td>
              </tr>
            } @empty {
              <tr><td colspan="8" class="empty">No RFQs yet.</td></tr>
            }
          </tbody>
        </table>
      </div>
...
export class RfqsListComponent implements OnInit {
  ...
  readonly rows = signal<RfqRequestDto[]>([]);
  readonly loading = signal(true);
  readonly error = signal<string | null>(null);

  async ngOnInit() {
    try { this.rows.set(await this.api.listRfqs()); }
    catch (e: any) { this.error.set(e?.message ?? 'Failed to load RFQs'); }
