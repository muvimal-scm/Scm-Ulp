...
public sealed class FreightService(FreightForwardingDbContext db, ITenantContext tenant, IClock clock) : IFreightService
{
    ...
    public async Task<ShipmentDto> CreateShipmentAsync(CreateShipmentRequest req, CancellationToken ct)
    {
        ...
        var now = clock.GetCurrentInstant();
        // CP13 v2 delta: pull trade direction from the linked booking, if any,
        // so the shipment list can filter Ocean/Air × Imp/Exp without re-joining.
        TradeDirection? direction = null;
        if (req.BookingId.HasValue)
        {
            direction = await db.Bookings.AsNoTracking()
                .Where(b => b.Id == req.BookingId.Value && b.TenantId == Tid)
                .Select(b => (TradeDirection?)b.TradeDirection)
    ...
    /* ===== CP13 v2 client doc delta — landed cost roll-up ===== */
    public async Task<LandedCostDto> GetLandedCostAsync(long shipmentId, CancellationToken ct)
    {
        // Verify shipment is visible to caller's tenant.
        var ship = await db.Shipments.AsNoTracking()
            .FirstOrDefaultAsync(s => s.Id == shipmentId && s.TenantId == Tid, ct)
            ?? throw new InvalidOperationException($"shipment {shipmentId} not found");

        var charges = await db.ChargeLines.AsNoTracking()
            .Where(c => c.TenantId == Tid && c.ShipmentId == shipmentId)
            .ToListAsync(ct);

        // Bucket by charge code prefix. The tenant's seeded charge_code values
        // determine which bucket a row falls in. Currency-mixing isn't ideal
        // but for a beta rollup we sum by amount in shipment's primary currency
        // (defaulted from first charge line, INR fallback).
        var currency = charges.FirstOrDefault()?.AmountCurrency ?? "INR";
        decimal Sum(Func<string, bool> match) =>
            charges.Where(c => c.AmountAmount.HasValue && match(c.ChargeCode))
                   .Sum(c => c.AmountAmount!.Value);

        bool IsFreight(string code)     => code.StartsWith("OCEAN_FREIGHT", StringComparison.OrdinalIgnoreCase)
                                        || code.StartsWith("AIR_FREIGHT",   StringComparison.OrdinalIgnoreCase)
                                        || code.StartsWith("FREIGHT",       StringComparison.OrdinalIgnoreCase)
                                        || code.Equals("FUEL",              StringComparison.OrdinalIgnoreCase);
        bool IsBrokerage(string code)   => code.StartsWith("BROKERAGE",     StringComparison.OrdinalIgnoreCase)
                                        || code.StartsWith("CUSTOMS_CLEAR", StringComparison.OrdinalIgnoreCase);
        bool IsDuty(string code)        => code.Equals("DUTY",              StringComparison.OrdinalIgnoreCase)
                                        || code.Equals("TAX",               StringComparison.OrdinalIgnoreCase)
                                        || code.Equals("TARIFF",            StringComparison.OrdinalIgnoreCase);
        bool IsAccessorial(string code) => code.StartsWith("DEMURRAGE",     StringComparison.OrdinalIgnoreCase)
                                        || code.StartsWith("DETENTION",     StringComparison.OrdinalIgnoreCase)
                                        || code.StartsWith("STORAGE",       StringComparison.OrdinalIgnoreCase)
                                        || code.StartsWith("HANDLING",      StringComparison.OrdinalIgnoreCase);

        var freight     = Sum(IsFreight);
        var brokerage   = Sum(IsBrokerage);
        var duty        = Sum(IsDuty);
        var accessorial = Sum(IsAccessorial);
        var other       = Sum(c => !IsFreight(c) && !IsBrokerage(c) && !IsDuty(c) && !IsAccessorial(c));
        var total       = freight + brokerage + duty + accessorial + other;

        return new LandedCostDto(shipmentId, currency, freight, brokerage, duty, accessorial, other, total);
    }
