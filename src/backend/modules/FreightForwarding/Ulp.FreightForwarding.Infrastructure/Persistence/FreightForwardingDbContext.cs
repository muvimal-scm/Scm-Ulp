...
public sealed class FreightForwardingDbContext(DbContextOptions<FreightForwardingDbContext> options) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder b)
    {
        ...
        b.Entity<Shipment>(e =>
        {
            ...
            e.Property(x => x.BookingId).HasColumnName("booking_id");
            e.Property(x => x.Mode).HasColumnName("mode").HasConversion(ModeToStr, StrToMode).HasMaxLength(15);
            // CP13 v2 delta: trade_direction backfilled from booking; nullable on legacy rows.
            e.Property(x => x.TradeDirection)
                .HasColumnName("trade_direction")
                .HasConversion(
                    v => v == null ? null : (v == TradeDirection.Import ? "IMPORT"
                                          : v == TradeDirection.Export ? "EXPORT"
                                          : v == TradeDirection.CrossTrade ? "CROSS_TRADE"
                                          : "DOMESTIC"),
                    v => v == null ? (TradeDirection?)null
                       : v == "IMPORT" ? TradeDirection.Import
                       : v == "EXPORT" ? TradeDirection.Export
                       : v == "CROSS_TRADE" ? TradeDirection.CrossTrade
