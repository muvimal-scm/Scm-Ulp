...
public static class ShipmentEndpoints
{
    ...
    {
        var g = app.MapGroup("/api/v1/freight-forwarding/shipments").WithTags("M5 · Shipments").RequireAuthorization();
        ...
        g.MapPost("/", async (
            ...
            [FromServices] IFreightService svc, CancellationToken ct) =>
        {
            try   { return Results.Created("/api/v1/freight-forwarding/shipments/", await svc.CreateShipmentAsync(req, ct)); }
        ...
        g.MapPost("/{id:long}/containers", async (long id,
            ...
            [FromServices] IFreightService svc, CancellationToken ct) =>
        {
            try   { return Results.Created($"/api/v1/freight-forwarding/shipments/{id}/containers", await svc.AddContainerAsync(id, req, ct)); }
        ...
        g.MapPost("/{id:long}/milestones", async (long id,
            ...
            [FromServices] IFreightService svc, CancellationToken ct) =>
        {
            try   { return Results.Created($"/api/v1/freight-forwarding/shipments/{id}/milestones", await svc.AddMilestoneAsync(id, req, ct)); }
            catch (InvalidOperationException ex) { return Results.BadRequest(new { error = ex.Message }); }
        });
        ...
        g.MapPost("/{id:long}/charges", async (long id,
            ...
            [FromServices] IFreightService svc, CancellationToken ct) =>
        {
            try   { return Results.Created($"/api/v1/freight-forwarding/shipments/{id}/charges", await svc.AddChargeAsync(id, req, ct)); }
            catch (InvalidOperationException ex) { return Results.BadRequest(new { error = ex.Message }); }
        });

        // CP13 v2 client doc delta — landed cost roll-up.
        g.MapGet("/{id:long}/landed-cost", async (long id,
            [FromServices] IFreightService svc, CancellationToken ct) =>
        {
            try   { return Results.Ok(await svc.GetLandedCostAsync(id, ct)); }
            catch (InvalidOperationException) { return Results.NotFound(); }
        });
        ...
        g.MapPost("/{id:long}/mbls", async (long id,
            ...
            [FromServices] IFreightService svc, CancellationToken ct) =>
        {
            try   { return Results.Created($"/api/v1/freight-forwarding/shipments/{id}/mbls", await svc.AddMblAsync(id, req, ct)); }
            catch (InvalidOperationException ex) { return Results.Conflict(new { error = ex.Message }); }
        });
        ...
        g.MapPost("/{id:long}/awbs", async (long id,
            ...
            [FromServices] IFreightService svc, CancellationToken ct) =>
        {
            try   { return Results.Created($"/api/v1/freight-forwarding/shipments/{id}/awbs", await svc.AddAwbAsync(id, req, ct)); }
            catch (InvalidOperationException ex) { return Results.Conflict(new { error = ex.Message }); }
        });
        ...
        g.MapPost("/{id:long}/memos", async (long id,
            ...
            [FromServices] IFreightService svc, CancellationToken ct) =>
        {
            try   { return Results.Created($"/api/v1/freight-forwarding/shipments/{id}/memos", await svc.AddMemoAsync(id, req, ct)); }
            catch (InvalidOperationException ex) { return Results.NotFound(new { error = ex.Message }); }
        });
        ...
        g.MapPost("/{id:long}/holds", async (long id,
            ...
            [FromServices] IFreightService svc, CancellationToken ct) =>
        {
            try   { return Results.Created($"/api/v1/freight-forwarding/shipments/{id}/holds", await svc.PlaceHoldAsync(id, req, raisedByUserId: null, ct)); }
            catch (InvalidOperationException ex) { return Results.NotFound(new { error = ex.Message }); }
        });
        ...
        g.MapPost("/{id:long}/reminders", async (long id,
            ...
            [FromServices] IFreightService svc, CancellationToken ct) =>
        {
            try   { return Results.Created($"/api/v1/freight-forwarding/shipments/{id}/reminders", await svc.AddReminderAsync(id, req, ct)); }
            catch (InvalidOperationException ex) { return Results.NotFound(new { error = ex.Message }); }
        });
