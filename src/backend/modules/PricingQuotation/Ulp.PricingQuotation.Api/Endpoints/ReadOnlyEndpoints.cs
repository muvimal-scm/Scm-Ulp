...
public static class ReadOnlyEndpoints
{
    ...
    {
        var s = app.MapGroup("/api/v1/pricing-quotation/surcharges").WithTags("M14 · Surcharges").RequireAuthorization();
        s.MapGet("/", async ([FromServices] IPricingService svc, CancellationToken ct) =>
            Results.Ok(await svc.ListSurchargesAsync(ct)));

        var c = app.MapGroup("/api/v1/pricing-quotation/contracts").WithTags("M14 · Contracts").RequireAuthorization();
