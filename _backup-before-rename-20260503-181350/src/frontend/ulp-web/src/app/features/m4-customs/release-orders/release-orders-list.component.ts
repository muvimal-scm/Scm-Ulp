...
import { ReleaseOrderDto } from '../shared/m4-types';

@Component({
  selector: 'ulp-m4-release-orders',
  standalone: true,
          ...
          <tbody>
            @for (r of rows(); track r.id) {
              <tr>
                ...
                <td><a [routerLink]="['/app/m4/entries', r.entryId]" class="link">#{{ r.entryId }}</a></td>
                <td>{{ r.issuedAt | slice:0:10 }}</td>
                <td>{{ r.cargoPickupAt ? (r.cargoPickupAt | slice:0:10) : '—' }}</td>
