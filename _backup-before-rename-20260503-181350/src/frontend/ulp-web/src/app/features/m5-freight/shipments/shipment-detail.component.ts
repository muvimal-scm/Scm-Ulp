...
import { HoldType, ReminderKind, ShipmentDetailDto, ShipmentHoldDto, ShipmentMemoDto, ShipmentReminderDto } from '../shared/m5-types';

@Component({
  selector: 'ulp-m5-shipment-detail',
  standalone: true,
  imports: [DecimalPipe, SlicePipe, FormsModule, RouterLink, MatIconModule, MatProgressSpinnerModule],
  ...
  template: `
    ...
    @if (data(); as d) {
      ...
      <section class="card">
        <h2>Memo notes ({{ memos().length }})</h2>
        @if (memos().length === 0) { <p class="muted">No memo notes recorded yet.</p> } @else {
          <ul class="memos">
            @for (m of memos(); track m.id) {
              <li [class.memo--pinned]="m.isPinned">
                <div class="memo__when">
                  {{ m.createdAt | slice:0:19 }}
                  @if (m.authorUserId) { <span class="muted">· user #{{ m.authorUserId }}</span> }
                  @if (m.isPinned) { <mat-icon class="pin">push_pin</mat-icon> }
                </div>
                <div class="memo__body">{{ m.body }}</div>
              </li>
            }
          </ul>
        }
        <div class="memo-add">
          <textarea rows="2" [(ngModel)]="newMemoBody" placeholder="Add an internal note…"></textarea>
