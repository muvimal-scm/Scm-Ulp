...
import { ManifestDto } from '../shared/m9-types';

@Component({
  selector: 'ulp-m9-manifests',
  standalone: true,
          ...
          <tbody>
            @for (m of rows(); track m.id) {
              <tr>
                ...
                <td>{{ m.routeId ? ('route #' + m.routeId) : '—' }}</td>
                <td>{{ m.courierType }}</td>
                <td class="num">{{ m.totalPieces ?? '—' }}</td>
                <td class="num">{{ m.totalWeightKg ? (m.totalWeightKg | number:'1.0-3') : '—' }}</td>
