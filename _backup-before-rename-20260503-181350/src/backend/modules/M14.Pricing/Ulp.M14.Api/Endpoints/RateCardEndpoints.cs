...
public static class RateCardEndpoints
{
    ...
    {
        var g = app.MapGroup("/api/v1/m14/rate-cards").WithTags("M14 · Rate Cards").RequireAuthorization();
        ...
        g.MapPost("/", async (
            ...
            [FromServices] IPricingService svc, CancellationToken ct) =>
        {
            try   { return Results.Created($"/api/v1/m14/rate-cards/", await svc.CreateRateCardAsync(req, ct)); }
        ...
        g.MapPost("/{id:long}/lines", async (long id,
            ...
            [FromServices] IPricingService svc, CancellationToken ct) =>
        {
            try   { return Results.Created($"/api/v1/m14/rate-cards/{id}/lines", await svc.AddRateCardLineAsync(id, req, ct)); }
            catch (InvalidOperationException ex) { return Results.Conflict(new { error = ex.Message }); }
        });
