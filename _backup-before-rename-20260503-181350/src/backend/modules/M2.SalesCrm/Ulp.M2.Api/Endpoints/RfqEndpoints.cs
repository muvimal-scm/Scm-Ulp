...
public static class RfqEndpoints
{
    ...
    {
        var g = app.MapGroup("/api/v1/m2/rfqs").WithTags("M2 · RFQs").RequireAuthorization();
        ...
        g.MapPost("/", async (
            ...
            [FromServices] ICrmService svc, CancellationToken ct) =>
        {
            try   { return Results.Created("/api/v1/m2/rfqs/", await svc.CreateRfqAsync(req, ct)); }
        ...
        g.MapPost("/{id:long}/lines", async (long id,
            ...
            [FromServices] ICrmService svc, CancellationToken ct) =>
        {
            try   { return Results.Created($"/api/v1/m2/rfqs/{id}/lines", await svc.AddRfqLineAsync(id, req, ct)); }
        ...
        g.MapPost("/{id:long}/responses", async (long id,
            ...
            [FromServices] ICrmService svc, CancellationToken ct) =>
        {
            try   { return Results.Created($"/api/v1/m2/rfqs/{id}/responses", await svc.AddRfqResponseAsync(id, req, ct)); }
            catch (InvalidOperationException ex) { return Results.BadRequest(new { error = ex.Message }); }
        });
