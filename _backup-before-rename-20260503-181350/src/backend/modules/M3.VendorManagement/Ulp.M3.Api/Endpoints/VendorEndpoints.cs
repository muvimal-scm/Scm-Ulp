...
public static class VendorEndpoints
{
    ...
    {
        var g = app.MapGroup("/api/v1/m3/vendors").WithTags("M3 · Vendors").RequireAuthorization();
        ...
        g.MapPost("/", async (
            ...
            [FromServices] IVendorService svc, CancellationToken ct) =>
        {
            try   { return Results.Created($"/api/v1/m3/vendors/", await svc.CreateAsync(req, ct)); }
        ...
        g.MapPost("/{id:long}/agreements", async (long id,
            ...
            [FromServices] IVendorService svc, CancellationToken ct) =>
        {
            try   { return Results.Created($"/api/v1/m3/vendors/{id}/agreements/", await svc.CreateAgreementAsync(id, req, ct)); }
            catch (InvalidOperationException ex) { return Results.Conflict(new { error = ex.Message }); }
        });
