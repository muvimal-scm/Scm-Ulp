...
using Period = Ulp.M17.Domain.Entities.Period;
...
/// (LLD §10). For Indian tenants this lands on IndiaTaxProvider which writes
    ...
    IServiceProvider sp) : IAccountsService
{
    ...
    // When no plugin is registered the service falls back to a zero-tax shape (sealed LLD §10.2
    // requires plugin failures to surface — but for Phase-1 demo without IRP/Avalara we simply
    ...
    public async Task<PeriodDto> ClosePeriodAsync(long id, CancellationToken ct)
    {
        ...
        // LLD §7.3 — move through SoftClose then Closed; for this Phase-1 path we
    ...
    public async Task<PeriodDto> ReopenPeriodAsync(long id, ReopenPeriodRequest req, CancellationToken ct)
    {
        ...
        // LLD §7.4 — privileged action; reason logged. RBAC enforcement (CFO+Auditor)
    ...
    public async Task<InvoiceDto> CreateInvoiceAsync(CreateInvoiceRequest req, CancellationToken ct)
    {
        ...
        // Line creation + tax compute via plugin (LLD §10.2)
    ...
    public async Task<InvoiceDto> PostInvoiceAsync(long id, CancellationToken ct)
    {
        ...
        // LLD §5.1: Plugin extension on Posted — IN invokes IRP for IRN; failures push back to Draft.
