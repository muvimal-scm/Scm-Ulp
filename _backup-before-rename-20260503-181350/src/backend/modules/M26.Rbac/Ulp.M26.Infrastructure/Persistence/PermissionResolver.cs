...
namespace Ulp.M26.Infrastructure.Persistence;

/// <summary>
/// Effective permissions per LLD §3.4–§3.7:
///   directRoles ∪ groupRoles → role_permission → permission
///   ∪ override.GRANT
///   ∖ override.DENY  (DENY supersedes per LLD §3.7)
/// Returns the dotted permission codes ("m1.party.read") for ASP.NET policy mapping.
/// </summary>
public sealed class PermissionResolver(M26DbContext db, IMemoryCache cache) : IPermissionResolver
{
    ...
    public async Task<IReadOnlySet<string>> GetEffectivePermissionsAsync(long userId, CancellationToken ct)
    {
        ...
        // Role-derived permissions: direct user→role plus group→role.
