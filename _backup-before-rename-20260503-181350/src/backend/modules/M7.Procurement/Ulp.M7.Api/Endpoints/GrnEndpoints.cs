...
public static class GrnEndpoints
{
    ...
    {
        var g = app.MapGroup("/api/v1/m7/grns").WithTags("M7 · Goods Receipts").RequireAuthorization();
        ...
        g.MapPost("/", async (
            ...
            [FromServices] IProcurementService svc, CancellationToken ct) =>
        {
            try   { return Results.Created("/api/v1/m7/grns/", await svc.CreateGrnAsync(req, ct)); }
