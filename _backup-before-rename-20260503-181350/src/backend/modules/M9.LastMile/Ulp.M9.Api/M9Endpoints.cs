...
using Ulp.M9.Domain.Entities;

namespace Ulp.M9.Api;

public static class M9Endpoints
{
    public static IEndpointRouteBuilder MapM9Endpoints(this IEndpointRouteBuilder app)
    {
        var bookings = app.MapGroup("/api/v1/m9/bookings").WithTags("M9 · Courier Bookings").RequireAuthorization();
        ...
        {
            try   { return Results.Created("/api/v1/m9/bookings/", await svc.CreateBookingAsync(req, ct)); }
        ...
        var routes = app.MapGroup("/api/v1/m9/routes").WithTags("M9 · Routes").RequireAuthorization();
        ...
        {
            try   { return Results.Created("/api/v1/m9/routes/", await svc.CreateRouteAsync(req, ct)); }
            catch (InvalidOperationException ex) { return Results.Conflict(new { error = ex.Message }); }
        });

        var manifests = app.MapGroup("/api/v1/m9/manifests").WithTags("M9 · Manifests").RequireAuthorization();
        manifests.MapGet("/", async ([FromServices] ILastMileService svc, CancellationToken ct) =>
            Results.Ok(await svc.ListManifestsAsync(ct)));

        var pods = app.MapGroup("/api/v1/m9/pods").WithTags("M9 · POD").RequireAuthorization();
        ...
        pods.MapPost("/", async ([FromBody] CreatePodRequest req, [FromServices] ILastMileService svc, CancellationToken ct) =>
            Results.Created("/api/v1/m9/pods/", await svc.RecordPodAsync(req, ct)));

        var cods = app.MapGroup("/api/v1/m9/cod").WithTags("M9 · COD").RequireAuthorization();
        ...
        cods.MapPost("/", async ([FromBody] CreateCodRequest req, [FromServices] ILastMileService svc, CancellationToken ct) =>
            Results.Created("/api/v1/m9/cod/", await svc.RecordCodAsync(req, ct)));

        var attempts = app.MapGroup("/api/v1/m9/attempts").WithTags("M9 · Attempts").RequireAuthorization();
        ...
        attempts.MapPost("/", async ([FromBody] CreateAttemptRequest req, [FromServices] ILastMileService svc, CancellationToken ct) =>
            Results.Created("/api/v1/m9/attempts/", await svc.RecordAttemptAsync(req, ct)));

        var zones = app.MapGroup("/api/v1/m9/zone-rates").WithTags("M9 · Zone Rates").RequireAuthorization();
        zones.MapGet("/", async (
            [FromServices] ILastMileService svc,
            [FromQuery] string? countryCode,
            [FromQuery] CourierType? courierType,
