...
namespace Ulp.M5.Infrastructure.Persistence;

public sealed class FreightService(M5DbContext db, ITenantContext tenant, IClock clock) : IFreightService
{
    ...
    public async Task<IReadOnlyList<ShipmentDto>> ListShipmentsAsync(ShipmentListQuery q, CancellationToken ct)
    {
        ...
        // MBL / HBL / Container number — resolve to shipment IDs
        ...
        // SCM Milestone 1+2 watchlist filter — restrict to current user's starred shipments.
        ...
        // Per-row IsStarred flag — fetch the user's full watchlist set in one query.
    ...
    /* ===== SCM Milestone 1 — internal memo notes ===== */
    ...
    public async Task<ShipmentHoldDto> PlaceHoldAsync(long shipmentId, PlaceHoldRequest req, long? raisedByUserId, CancellationToken ct)
    {
        var s = await db.Shipments.AsNoTracking().FirstOrDefaultAsync(x => x.Id == shipmentId && x.TenantId == Tid, ct)
            ?? throw new InvalidOperationException($"shipment {shipmentId} not found");
        // Don't double-stack the same active hold type — return the existing one.
    ...
    /// Email dispatch is intentionally NOT wired here yet — the visible state
